...
def deep_wb(image, task='all', net_awb=None, net_t=None, net_s=None, device='cpu', s=656):
    ...
    image_resized = cv2.resize(image, (round(img_width / max_img_dim * s), round(img_height / max_img_dim * s)))
    w, h = image_resized.shape[1], image_resized.shape[0]
    if w % 2 ** 4 == 0:
        new_size_w = w
    else:
    ...
    if not ((w, h) == inSz):
        image_resized = cv2.resize(image_resized, inSz)
